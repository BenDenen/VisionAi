...
// range checks on these (toFixed/toFloat, etc)
        ...
        static_assert(std::is_pod<UnsafeFloatLatitude>(),
                      "UnsafeFloatLatitude is not a valid alias");
        static_assert(std::is_pod<UnsafeFloatLongitude>(),
 ...
 */
        inline FixedLatitude toFixed(const FloatLatitude floating) {
            ...
            const auto fixed = static_cast<std::int32_t>(std::round(
                    latitude * COORDINATE_PRECISION));
 ...
 */
        inline FixedLongitude toFixed(const FloatLongitude floating) {
            ...
            const auto fixed = static_cast<std::int32_t>(std::round(
                    longitude * COORDINATE_PRECISION));
 ...
 */
        struct Coordinate {
            ...
            Coordinate(const FixedLongitude lon_, const FixedLatitude lat_) : lon(lon_),
                                                                              lat(lat_) {}

            template<class T>
            Coordinate(const T &coordinate) : lon(coordinate.lon), lat(coordinate.lat) {
 ...
 */
        struct FloatCoordinate {
            ...
            FloatCoordinate(const FloatLongitude lon_, const FloatLatitude lat_) : lon(lon_),
                                                                                   lat(lat_) {}
